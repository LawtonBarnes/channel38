...
class Segment(SegmentParent):
    ...
    def get_story(self, url):
        ...
        story = [p for p in story if not p.startswith(('AP Photo', 'Photo:', 'FILE -', 'FILE—', 'FILE -'))]
        story = [p for p in story if not p.endswith(('AP)', '(AP)', 'Associated Press'))]
        if len(story) == 0:
            return None
        return story
    ...
    def refresh_data(self):
        ...
        for story in story_divs:
            ...
            skip_patterns = ['/photo-gallery/', '/live/', '/video/', '/news-quiz/']
    ...
    def show_headlines(self, num_headlines):
        ...
        for item in self.data['items']:
            if shown >= num_headlines:
                break
            if 'AP Photo' in item['headline']:
                continue
            self.d.newline(self.d.beat_delay)
            self.d.set_color('\033[35m')
